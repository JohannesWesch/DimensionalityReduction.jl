...
function block_elimination_nnenum(onnx_input, output; doreduction=true, method=0,
    ...
    epsilons = ["benchmarks/digits/dim784/prop_7_0.004.vnnlib",
    ...
    epsilons_text = [0.004, 0.005, 0.006, 0.007, 0.008]

    runtime_customoverall = vec(readdlm("23-customoverall.txt", '\t', Float64, '\n'))
    runtime_custom = vec(readdlm("23-custom.txt", '\t', Float64, '\n'))
    runtime_unitvectoroverall = vec(readdlm("23-unitvectoroverall.txt", '\t', Float64, '\n'))
    runtime_unitvector = vec(readdlm("23-unitvector.txt", '\t', Float64, '\n'))
    runtime_no = vec(readdlm("23-no.txt", '\t', Float64, '\n'))
    color_vec1 =  vec(readdlm("23-colorvec1.txt", '\t', String, '\n'))
    color_vec2 =  vec(readdlm("23-colorvec1.txt", '\t', String, '\n'))
    dummy = zeros(15,)

    pertubation = readdlm("PertubationA.txt", '\t', Float64, '\n')
    ...
    #=for (i, epsilon) in enumerate(epsilons)
        ...
        open("23-customoverall.txt", "w") do io
            writedlm(io, runtime_customoverall)
        end
        open("23-custom.txt", "w") do io
            writedlm(io, runtime_custom)
        end
        open("23-colorvec1.txt", "w") do io
    ...
    #=for (i, epsilon) in enumerate(epsilons)
        ...
        open("23-unitvectoroverall.txt", "w") do io
            writedlm(io, runtime_unitvectoroverall)
        end
        open("23-unitvector.txt", "w") do io
            writedlm(io, runtime_unitvector)
        end
        open("23-colorvec2.txt", "w") do io
