...
function block_elimination_nnenum(onnx_input, output; doreduction=true, method=0,
    ...
    for (i, dim) in enumerate(dims)
        result = reduce(onnx_input, "benchmarks/digits/dim64/prop_5_1.00.vnnlib", output; doreduction, method=2, d_to_reduce=dim,
    ...
    gridwidth=0.1),
    yaxis2=attr(title="Runtime (s)", linecolor="black", nticks=4, type="log", side="right", overlaying="y"),
