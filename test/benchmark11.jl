...
function block_elimination_nnenum(onnx_input, output; doreduction=true, method=0,
    ...
    for (i, dim) in enumerate(dims)
        if i == 0
            result = reduce(onnx_input, "benchmarks/digits/dim64/prop_5_0.60.vnnlib", output; doreduction=false, method=2, d_to_reduce=dim,
            vnnlib, nnenum, factorization=3, dorefinement)
        else
            result = reduce(onnx_input, "benchmarks/digits/dim64/prop_5_0.60.vnnlib", output; doreduction, method=2, d_to_reduce=dim,
            vnnlib, nnenum, factorization=3, dorefinement)
        end
        runtime[i] = result[4]
        stars[i] = result[3]
