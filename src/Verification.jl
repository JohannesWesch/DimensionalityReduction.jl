...
#=function generate_input_spec(bounds, matrix, bias, ninputs):
	init_star = lp_star.LpStar(np.eye(ninputs, dtype=np.float32), np.zeros(ninputs, dtype=np.float32), bounds)
...
py"""
import sys
sys.path.insert(0, "../nnenum")
"""
@pyinclude("nnenum/enumerate.py")

py"enumerate_network"(input_spec, network, output_spec)